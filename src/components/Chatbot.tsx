...
export const Chatbot = ({ onClose }: ChatbotProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 flex flex-col">
      ...
      <div className="bg-yellow-50 border-b border-yellow-200 px-2 sm:px-4 py-2 text-center">
        <p className="text-xs text-yellow-800">
          For best experience, use Chrome, Brave, or other modern browsers
        </p>
      </div>
      {/* Header */}
      <div className="bg-blue-600 text-white p-3 sm:p-4 flex items-center justify-between shadow-md shrink-0">
        <div className="flex items-center space-x-3">
          <Bot className="h-6 w-6" />
          <div>
            <h1 className="text-base sm:text-lg font-semibold">PolyPros</h1>
            <p className="text-xs sm:text-sm text-blue-100">Study Assistant</p>
          </div>
        </div>
        <div className="flex items-center space-x-2">
          <Button
            ...
            size="sm"
            onClick={clearChat}
            className="text-white hover:bg-blue-700 h-8 w-8 sm:h-10 sm:w-10 p-0"
            title="Clear chat"
          >
      ...
      <div className="flex-1 flex flex-col min-h-0">
        <ScrollArea className="flex-1 px-2 sm:px-4 py-4 sm:py-6">
          <div className="max-w-4xl mx-auto space-y-6">
            {messages.map((message) => (
              ...
              >
                <div className={`flex items-start space-x-2 sm:space-x-3 max-w-[90%] sm:max-w-[85%] ${message.isBot ? "" : "flex-row-reverse space-x-reverse"}`}>
                  ...
                  <div className={`rounded-2xl px-3 sm:px-4 py-2 sm:py-3 transition-all duration-300 animate-fade-in ${
                    message.isBot
                      ? message.isError
                    ...
                    <div className="text-xs sm:text-sm leading-relaxed whitespace-pre-wrap animate-scale-in">
                      {message.text}
                    </div>
            ...
            {isTyping && (
              <div className="flex justify-start animate-fade-in">
                <div className="flex items-start space-x-2 sm:space-x-3 max-w-[90%] sm:max-w-[85%]">
                  ...
                  <div className="rounded-2xl px-3 sm:px-4 py-2 sm:py-3 bg-gray-100 shadow-sm animate-scale-in">
        ...
        {/* Input Area */}
        <div className="border-t bg-white p-3 sm:p-4 shrink-0 safe-area-inset-bottom">
          <div className="max-w-4xl mx-auto">
            <div className="flex space-x-2 sm:space-x-3 mb-2">
              <Input
                ...
                className="flex-1 rounded-full border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm sm:text-base"
                disabled={isTyping}
              />
              <Button 
                onClick={() => handleSendMessage()} 
                className="bg-blue-600 hover:bg-blue-700 rounded-full h-10 w-10 sm:h-12 sm:w-12 p-0 shrink-0"
            ...
            <p className="text-xs text-gray-500 text-center">
              ...
              <span className="text-gray-400 hidden sm:inline">Tip: Ask for "detailed explanation" for more info</span>
            </p>
          </div>
        </div>
      </div>
